...
class System extends Model{
	...
	{
		$sql = new Sql();
		$result = $sql->select("SELECT * FROM st_contaparam p 
								INNER JOIN st_comissions cm ON p.idconta_contaparam = cm.id_conta_comission
								INNER JOIN st_conta c ON c.id_Conta = p.idconta_contaparam  
								WHERE p.idconta_contaparam = :id", array(":id"=>$idconta));
	...
	public function getConfBroker($idconta)
	{
		...
		return $results = $sql->select("SELECT * FROM st_conta c 
										INNER JOIN st_comissions cm 
										ON c.id_Conta = cm.id_conta_comission 
